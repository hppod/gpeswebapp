...
export class HomeValidator {
    ...
    /**Função que valida se o título digitado será único no banco de dados. */
    checkUniqueTitulo(): AsyncValidatorFn {
        return control => control.valueChanges
            .pipe(
                debounceTime(400),
                distinctUntilChanged(),
                switchMap(value => this._service.checkUniqueHomeTitulo(value)),
                map((response) => {
                    if (response['result'] == 0 && control.value != null && control.value != '') {
                        return null
                    } else {
                        return { 'homeTituloAlreadyExists': true }
                    }
                }),
                first())
    }
