...
export class NoticiasService {
    ...
    getNoticiaByTitle(title: string, modifier: string): Observable<HttpResponse<Noticia>> {
        return this.http.get<Noticia>(`${GPESWebApi}/${modifier}/noticia/listar-um/${title}`, { observe: 'response' })
    }

    deleteNoticia(id): Observable<HttpResponse<Noticia>> {
        return this.http.delete<Noticia>(`${GPESWebApi}/authenticated/noticia/apagar/${id}`, { params: id, observe: 'response' })
    }

    getDataByTitle(title: string): Observable<HttpResponse<Noticia>> {
        return this.http.get<Noticia>(`${GPESWebApi}/authenticated/noticia/getdata/${title}`, { observe: 'response' })
    }
    ...
    postNoticia(formData: FormData) {
        return this.http.post<FormData>(`${GPESWebApi}/authenticated/noticia/criar`, formData, { reportProgress: true, observe: 'events' })
    }
